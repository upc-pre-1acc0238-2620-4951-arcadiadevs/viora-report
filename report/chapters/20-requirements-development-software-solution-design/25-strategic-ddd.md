...
\begin{itemize}
    ...
    \item \textbf{Anticorruption Layer (ACL - Capa Anticorrupción):} Se interpone de manera obligatoria en los puntos de contacto con sistemas externos: la pasarela de pagos (\textit{Mercado Pago Checkout Pro}), el servicio agrometeorológico (\textit{SENAMHI Weather API}), el proveedor cartográfico (\textit{Mapbox GIS}) y el servicio de correo (\textit{Transactional Mail Service}). La ACL aísla al dominio de Viora frente a cambios de esquemas, códigos de respuesta propietarios o estructuras de datos ajenas, traduciendo cada interacción a comandos y eventos nativos de la plataforma.
\end{itemize}
...
\noindent \textbf{Evaluación y Justificación de Diseños Alternativos:} Durante el proceso de modelado estratégico se analizaron dos hipótesis de diseño alternativas que fueron formalmente desestimadas tras una rigurosa evaluación de acoplamiento, cohesión y mantenibilidad:
