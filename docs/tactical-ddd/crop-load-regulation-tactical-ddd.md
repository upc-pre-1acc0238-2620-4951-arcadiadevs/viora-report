...
- **`OnThinningWindowClosedEventHandler`** (POL10), atributo `prescriptions`: `handle(ThinningWindowClosedByPitHardening): void`. Expira las prescripciones pendientes de la parcela. Consumo interno del propio contexto.
...
| `PhenologicalScheduler` | Tarea programada que consulta `PhenologicalSignalPort` y despacha `CloseThinningWindowByPhenology` (`CMD27`) sobre las parcelas cuya lignificación se confirma. **Ver la nota de pendientes al cierre del documento: hoy ningún contexto produce esa señal.** |
...
|---|---|---|---|
| P1 | **La señal de lignificación del carozo no la produce ningún contexto.** `CMD27` declara iniciador «Sensor de Grados-Día Fenológicos» y ese sensor no existe: el endurecimiento del endocarpio aparece en los nueve documentos del event storming, pero en la capa táctica **solo en este documento, como consumo**. Phenology no lo modela ni lo emite. | `Paso5_commands.md` (ficha de `CMD27`); ausencia verificada en los nueve tácticos | **Crítico.** Sin esa señal, `CMD27` nunca se dispara, `EV43` nunca se emite, `POL10` nunca corre y la **invariante 3 —el cierre biológico irrevocable— carece de mecanismo de activación**. `POL09` además exige `isPitHardened == false`, un dato que hoy nadie puede producir. |
